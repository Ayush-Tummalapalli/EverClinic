...
fun AddAppointmentScreen(navController: NavController) {
    ...
    val viewModel: AddAppointmentViewModel = viewModel(factory = ViewModelFactory(patientDao, doctorDao, appointmentDao, treatmentDao))
    ...
    ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = !expanded }) {
        ...
        ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
            patients.forEach { patient ->
                DropdownMenuItem(text = { Text(patient.name) }, onClick = {
    ...
    ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = !expanded }) {
        ...
        ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
            doctors.forEach { doctor ->
                DropdownMenuItem(text = { Text(doctor.name) }, onClick = {
...
@Composable
fun DateTimePickers(date: String, onDateChange: (String) -> Unit, time: String, onTimeChange: (String) -> Unit) {
    val context = LocalContext.current
    ...
    val timePickerDialog = TimePickerDialog(
        context,
        { _, hourOfDay, minute ->
            onTimeChange(String.format(Locale.getDefault(), "%02d:%02d", hourOfDay, minute))
        },
